...
from NETWORK import TheModelClass
...
class Replica(G3PCX, multiprocessing.Process):
    def __init__(self, num_samples, burn_in, population_size, topology, train_data, test_data, directory, temperature, swap_interval, parameter_queue, problem_type,  main_process, event, max_limit=(-5), min_limit=5):
        ...
        self.network=TheModelClass(topology)
    ...
    def fitness_function(self, x):
        fitness = self.neural_network.evaluate_fitnessneural(x)
    ...
    @staticmethod
    def multinomial_likelihood(network, data, weights, temperature):
        y = data[:, network.topology[0]: network.topology[0] + network.topology[2]]
        fx = network.forward(data, weights)
    ...
    @staticmethod
    def gaussian_likelihood(network, data, weights, tausq, temperature):
        desired = data[:, network.topology[0]: network.topology[0] + network.topology[2]]
        prediction = network.forward(data, weights)
